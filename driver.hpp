...
namespace miniFE {
    ...
    int driver(const Box& global_box, Box& my_box, Parameters& params, YAML_Doc& ydoc)
    {
...
//                verify_result = verify_solution(mesh, x, tolerance, verify_whole_domain);
        ...
        if (myproc == 0) {
            ...
            // POSIT FIXME:  casting to double?
            ydoc.get(title)->add("Final Resid Norm", (double)rnorm);
