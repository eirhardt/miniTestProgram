...
//
    ...
    {
#if USE_POSITX
        return sw::unum::sqrt((2*p+1)*(2*p+1)*PI_SQR + (2*q+1)*(2*q+1)*PI_SQR);
#else
...
#if USE_POSITX
            const Scalar sin_py = sw::unum::sin(p21y)/(2*p+1);
#else
...
#if USE_POSITX
                const Scalar sin_qz = sw::unum::sin(q21z)/(2*q+1);
#else
...
#if USE_POSITX
                const Scalar sinh1 = sw::unum::sinh(l*x);
                const Scalar sinh2 = sw::unum::sinh(l);
#else
